/**
 * Sistema de busca de CPF/CNPJ com injeção de botão DENTRO de cada iframe de conversa.
 * Versão final com "vigia" que reinjeta o botão se a interface do chat for redesenhada.
 */
...
class ButtonInjector {
    ...
    inject(iframeDoc) {
        ...
        if (!targetContainer) {
            console.error(`Não foi possível encontrar o local para injetar o botão no chat.`);
            return;
        }

        const button = iframeDoc.createElement('button');
        // Usamos uma CLASSE para identificação, pois IDs devem ser únicos e o ID some
        // quando a interface é redesenhada.
        ...
        console.log(`✅ Botão de busca injetado com sucesso no chat!`);
    }
}

class DocumentFinderSystem {
    constructor() {
    ...
    findActiveIframe() {
        const strategy1 = () => {
            const activeWrappers = document.querySelectorAll('div.app-view-stack.conversation-v2[aria-hidden="false"]');
            if (activeWrappers.length > 0) {
                const lastWrapper = activeWrappers[activeWrappers.length - 1];
                const appViewStack = lastWrapper.querySelector('app-view-stack');
                if (appViewStack && appViewStack.shadowRoot) return appViewStack.shadowRoot.querySelector('iframe[src*="messaging-gadget"]');
            }
            return null;
        };
        const strategy2 = () => {
            const activeLegacyInstances = document.querySelectorAll('div.app-instance.legacy[aria-hidden="false"]');
            if (activeLegacyInstances.length > 0) {
                const lastInstance = activeLegacyInstances[activeLegacyInstances.length - 1];
                return lastInstance.querySelector('iframe[src*="messaging-gadget"]');
            }
            return null;
        };
    ...
    /**
     * LÓGICA DO VIGIA
     * A cada ciclo, ele encontra o iframe ativo e verifica se o botão já existe LÁ DENTRO.
     * Se não existir (porque a interface foi redesenhada), ele o injeta novamente.
     */
    scanAndEnsureButtonExists() {
        const activeIframe = this.findActiveIframe();
        if (!activeIframe) return;

        try {
            const iframeDoc = activeIframe.contentDocument;
            // Aguarda o documento do iframe estar minimamente pronto para ser inspecionado
            ...
            if (!actionBar) {
                // Barra de ações ainda não foi renderizada, tenta de novo no próximo ciclo.
                return;
            }

            const buttonExists = actionBar.querySelector('.document-search-btn');
            if (!buttonExists) {
                // O botão não está na barra de ações atual, então injeta!
                console.log(`🔎 Conversa ativa não possui o botão. Injetando...`);
                this.buttonInjector.inject(iframeDoc);
            }

        } catch (e) {
            // Ignora erros de segurança que podem acontecer durante a transição de iframes
        }
    }

    initialize() {
        console.log('Inicializando Sistema de Busca de Documentos (Modo Vigia)...');
        // O vigia agora verifica o estado do botão a cada 1 segundo.
        setInterval(() => this.scanAndEnsureButtonExists(), 1000);
        console.log('📡 Vigia de botão ativado.');
    }
}
...
(function() {
    ...
    console.log('🚀 SISTEMA DE BUSCA DE CPF/CNPJ CARREGADO');
    console.log('='.repeat(60));
    if (!(window.DocumentFinderSystem instanceof DocumentFinderSystem)) {
        window.DocumentFinderSystem = new DocumentFinderSystem();
        window.DocumentFinderSystem.initialize();
    } else {
